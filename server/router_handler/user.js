...
exports.regUser = (req, res) => {
  ...
  db.query(sqlStr, userinfo.username, (err, results) => {
    // 查询数据库中该账号是否存在
    if (err) {
      return res.send({ code: 1, message: "数据库链接失败 ", err });
    }
    if (results.length > 0) {
      // 查询结果长度不为0则表示账号已存在
      return res.send({ code: 1, message: "注册失败,账号已存在" });
    }
    // (2)将用户密码加密
    ...
    db.query(
      ...
      (err, results) => {
        ...
        if (results.affectedRows !== 1) {
          // 新建用户失败
          return res.send({ code: 1, message: "用户注册失败" });
        }
        // 新建用户成功
...
exports.login = (req, res) => {
  ...
  db.query(sql, userinfo.username, (err, results) => {
    ...
    if (!compare) return res.send({ code: 1, message: "账号或密码错误" });
    // (2)登录成功后生成token令牌
