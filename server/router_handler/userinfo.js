...
exports.getUserInfo = (req, res) => {
  // 定义查询语句(排除密码)
  const sql =
    "select id,username,nickname,user_pic,balance from users where id=?";
  db.query(sql, req.auth.id, (err, results) => {
    if (err) return res.send({ code: 1, message: "数据库操作失败" });
    if (results.length < 1) {
      return res.send({ code: 1, message: "未查询到用户信息" });
    }
    res.send({
      code: 0,
      message: "获取用户信息成功",
...
exports.updatePassword = (req, res) => {
  ...
  db.query(sql, req.auth.id, (err, results) => {
    ...
    if (!compare) return res.send({ code: 1, message: "原密码错误" });
    // 修改用户密码
    ...
    db.query(sql, [newPsw, req.auth.id], (err, results) => {
      if (err) return res.send({ code: 1, message: "数据库操作失败" });
      if (results.affectedRows !== 1) {
        return res.send({ code: 1, message: "修改密码失败" });
      }
...
exports.updateAvatar = (req, res) => {
  // 判断是否成功保存图片
  if (!req.file) {
    return res.send({ code: 1, message: "图片格式必须为jpeg,jpg,png" });
  }
  // 取出头像路径 注意 \\为一个反斜线
  ...
  db.query(sql, [avatar, req.auth.id], (err, results) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ code: 1, message: "数据库操作失败" });
    }
    if (results.affectedRows !== 1) {
      return res.status(400).json({ code: 1, message: "修改头像失败" });
    }
    res.json({
      code: 0,
      message: "修改头像成功",
